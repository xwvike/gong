...
**`default` 主题的 LED 信号板从来没滚动过**这条是独立的：它的滚动建立在 rAF 上，当时实测 `累计rAF帧数=0`。既然 rAF 现在是活的，值得重新验一遍——还没做。`themes/noise/index.html` 里也抄了一份旧结论的注释，同样过期，但 noise 本身按 10fps 出画是它自己要的效果（撕裂感），行为不用改。
